...
#define IsIOSV(vesion) ([[[UIDevice currentDevice] systemVersion] floatValue] == vesion ? YES : NO)

@implementation UIImage (JLImageKit)
...
+(UIImage *)jl_imageContentFileWithName:(NSString *)imageName ofType:(NSString *)type
{
    if([UIScreen mainScreen].scale>=2)
    {
        ...
        return [UIImage jl_imageWithData2:imageData scale:2];
    }
    else{
...
+(UIImage *)jl_imageWithImage:(UIImage *)image ratioToSize:(CGSize)newSize
{
    ...
    return [self jl_imageWithImage:image scaledToSize:CGSizeMake(width,height)];
}
//按最短边 等比压缩
+(UIImage *)jl_imageWithImage:(UIImage *)image ratioCompressToSize:(CGSize)newSize
{
    ...
    {
        return [self jl_imageWithImage:image ratioToSize:newSize];
    }
}
#pragma mark 添加圆角
...
{
    return [UIImage jl_imageWithImage:image pixelOperationBlock:^(UInt8 *redRef, UInt8 *greenRef, UInt8 *blueRef) {
...
+(UIImage *)jl_imageWithImage:(UIImage *)image pixelOperationBlock:(void(^)(UInt8 *redRef, UInt8 *greenRef, UInt8 *blueRef))block
{
    if(block == nil)
        return image;
